...
PYBIND11_MODULE(calcengine, m) {
    ...
    m.def("get_signature", [](const string& expr_str, const string& var) {
        try {
            ExprPtr e = parse(expr_str);
            return getSignature(simplify(e), var);
        } catch (...) {
            return string("PARSE_ERROR");
        }
    }, "Get structural hash signature", py::arg("expr"), py::arg("var") = "x");
