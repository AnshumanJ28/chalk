...
ExprPtr differentiate(const ExprPtr& e, const string& v) {
    switch (e->op) {
        ...
        case Op::SINH:
            return mul(fn(Op::COSH, e->args[0]), differentiate(e->args[0], v));
        case Op::COSH:
            return mul(fn(Op::SINH, e->args[0]), differentiate(e->args[0], v));
        case Op::TANH:
            return divi(differentiate(e->args[0], v),
                       powr(fn(Op::COSH, e->args[0]), num(2)));
        case Op::ATANH:
            return divi(differentiate(e->args[0], v),
                       sub(num(1), powr(e->args[0], num(2))));
        case Op::SEC:
            return mul(mul(fn(Op::SEC, e->args[0]), fn(Op::TAN, e->args[0])),
                      differentiate(e->args[0], v));
        case Op::CSC:
            return neg(mul(mul(fn(Op::CSC, e->args[0]), fn(Op::COT, e->args[0])),
                          differentiate(e->args[0], v)));
        case Op::COT:
            return neg(divi(differentiate(e->args[0], v),
                           powr(fn(Op::SIN, e->args[0]), num(2))));
